...
export default {
  ...
  methods: {
    ...
    async getCategory() {
      try {
        let res = await this.$axios.post("/api/product/category/list", {})
        const val = {
          category_id: 0,
          category_name: "全部"
        };
        const cate = res.data.data;
        // 使用该方法将数据添加到数组的起始位置,保证获取到的categoryList首位是“全部”
        cate.unshift(val);
    ...
    // 向后端请求全部商品或分类商品数据
    async getData() {
      // 如果分类列表为空则请求全部商品数据，否则请求分类商品数据，categoryID为0则代表请求所有分类数据
